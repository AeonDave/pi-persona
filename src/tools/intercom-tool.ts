...
export function registerIntercomTool(pi: ExtensionAPI, d: IntercomToolDeps): void {
	...
	const IntercomToolParams = Type.Object({
		...
		to: Type.Optional(Type.String({ maxLength: MAX_INTERCOM_REF_CHARS, description: "result/steer/stop/peek: the async run id (e.g. 'run-1') · wait: an async run id; omit it to join all running and pending completions · send: the child handle (from `list`)" })),
		...
		timeoutMs: Type.Optional(Type.Number({ description: "wait: max ms to hold your turn (default 600000; values below 1000 clamp to 1000, values above 600000 clamp to 600000) — on timeout you get what settled + what's still running" })),
	});
	pi.registerTool({
		name: "intercom",
		label: "Intercom",
		description: [
			...
			"while child-engine (MCP/worktree) async runs receive it as a brokered follow-up.",
