...
test("delivers with attribution from the RESOLVED label (not the envelope's self-report) and fenced", () => {
	...
	assert.doesNotMatch((out as any).deliver, /SPOOFED/, "self-reported name is never trusted for attribution");
		assert.match((out as any).deliver, /> do X/, "text is fenced by the canonical peer fence");
...
test("a budget-rejected message can be retried after the window without becoming a duplicate", () => {
	let now = 0;
	const d = {
		budget: new SenderBudget({ windowMs: 1_000, maxMsgs: 1, maxBytes: 1_000, now: () => now }),
		seen: new SeenMessages({ ttlMs: 10_000, now: () => now }),
		injectMaxBytes: 50,
	};
	assert.ok("deliver" in buildInboundDelivery(msg({ msg_id: "prior" }), "x", d));
	assert.deepEqual(buildInboundDelivery(msg(), "x", d), { drop: "budget" });
	now = 1_001;
	const retry = buildInboundDelivery(msg(), "x", d);
	assert.ok("deliver" in retry, "the sender may retry once its budget window resets");
});
