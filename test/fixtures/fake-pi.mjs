...
function run(task) {
	if (task.includes("[sleep]")) {
		...
		emit({
			...
			message: {
				role: "assistant",
				content: [{ type: "text", text: `PI_PERSONA_DISABLE=${process.env.PI_PERSONA_DISABLE ?? "unset"} PI_PERSONA_CHILD=${process.env.PI_PERSONA_CHILD ?? "unset"} PI_PERSONA_BUS=${process.env.PI_PERSONA_BUS ?? "unset"} PI_PERSONA_HANDLE=${process.env.PI_PERSONA_HANDLE ?? "unset"} PI_PERSONA_PEERS=${process.env.PI_PERSONA_PEERS ?? "unset"}` }],
				model: "stub/model",
				stopReason: "end",
				usage: { input: 1, output: 1, cost: { total: 0 }, totalTokens: 2 },
			},
