...
- **Cross-process broker — on by default; `PI_PERSONA_BROKER=off` restores pre-broker spawn.**
  Extends both layers (steer + the comm plane) to sub-agents that don't run in-process:
  `PI_PERSONA_ENGINE=child` runs, every `isolation: worktree` leg, and every `mcp: true` leg (those
  always use the child engine). A session-scoped relay (POSIX socket / Windows named pipe) connects
  the child straight into your supervisor surface. The host starts lazily on the first child-engine
  build. Design: [`docs/ARCHITECTURE.md`](docs/ARCHITECTURE.md#the-comm-plane-in-practice).
...
> engine, which DOES connect (to the same servers in `~/.pi/agent/mcp.json`). Steer and
> `contact_supervisor` ride the session broker (on by default). The child gets its own
...
Everything below is data — drop the files in (discovery: builtin < `~/.pi/agent/persona/` < project `.pi/`)
